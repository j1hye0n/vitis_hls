...
{
  unsigned int i;
  VITIS_LOOP_363_1: for(i=0;i<25;i++)
    s[i] = 0;
}
# 380 "../fips202.c"
                                  ...
                                  size_t inlen)
{
  ...
  VITIS_LOOP_388_1: while(pos+inlen >= r) {
    VITIS_LOOP_389_2: for(i=pos;i<r;i++)
  ...
  VITIS_LOOP_396_3: for(i=pos;i<pos+inlen;i++)
    s[i/8] ^= (uint64_t)*in++ << 8*(i%8);
                                   ...
                                   unsigned int r)
{
  ...
  VITIS_LOOP_441_1: while(outlen) {
    if(pos == r) {
      KeccakF1600_StatePermute(s);
      pos = 0;
    }
    VITIS_LOOP_446_2: for(i=pos;i < r && i < pos+outlen; i++)
      *out++ = s[i/8] >> 8*(i%8);
    outlen -= i-pos;
    pos = i;
  }
                               ...
                               uint8_t p)
{
  ...
  VITIS_LOOP_477_1: for(i=0;i<25;i++)
    s[i] = 0;
                                 ...
                                 unsigned int r)
{
  ...
  VITIS_LOOP_544_1: while(nblocks) {
    KeccakF1600_StatePermute(s);
    VITIS_LOOP_546_2: for(i=0;i<r/8;i++)
      store64(out+8*i, s[i]);
    out += r;
    nblocks -= 1;
  }
...
void pqcrystals_kyber_fips202_ref_sha3_256(uint8_t h[32], const uint8_t *in, size_t inlen)
{
  ...
  KeccakF1600_StatePermute(s);
  VITIS_LOOP_790_1: for(i=0;i<4;i++)
#pragma HLS pipeline off
 store64(h+8*i,s[i]);
}
# 804 "../fips202.c"
void pqcrystals_kyber_fips202_ref_sha3_512(uint8_t h[64], const uint8_t *in, size_t inlen)
{
  ...
  KeccakF1600_StatePermute(s);
  VITIS_LOOP_811_1: for(i=0;i<8;i++)
    store64(h+8*i,s[i]);
}

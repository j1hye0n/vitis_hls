...
static void store64(uint8_t x[8], uint64_t u) {
    //#pragma HLS ARRAY_PARTITION variable=x factor=8 //diff x
  unsigned int i;

  for(i=0;i<8;i++)
    #pragma HLS pipeline //-> no change
    x[i] = u >> 8*i;
}
...
{
    //#pragma HLS ARRAY_PARTITION variable=state factor=25
    //#pragma HLS INLINE
        int round;
...
state_permute : for(round = 0; round < NROUNDS; round += 2) {
#pragma HLS pipeline
                                  ...
                                  size_t inlen)
{
  ...
  while(pos+inlen >= r) {
    absorb_loop:for(i=pos;i<r;i++)
    #pragma HLS unroll //diff x
    //#pragma HLS pipeline -> diff x
  ...
  absorb_loop2:for(i=pos;i<pos+inlen;i++)
  //#pragma HLS pipeline //diff x
    s[i/8] ^= (uint64_t)*in++ << 8*(i%8);
                                   ...
                                   unsigned int r)
{
  ...
  while(outlen) {
    ...
    squeeze_looop:for(i=pos;i < r && i < pos+outlen; i++)
    //#pragma HLS pipeline //diff x
      *out++ = s[i/8] >> 8*(i%8);
    outlen -= i-pos;
    pos = i;
...
#pragma HLS ARRAY_PARTITION variable=local_block complete
                                 ...
                                 unsigned int r)
{
  ...
  while(nblocks) {
    KeccakF1600_StatePermute(s);
    squeezeblocks_loop:for(i=0;i<r/8;i++)
    //#pragma HLS unroll //no change
    //#pragma HLS pipeline //diff x
      store64(out+8*i, s[i]);
    out += r;
    nblocks -= 1;

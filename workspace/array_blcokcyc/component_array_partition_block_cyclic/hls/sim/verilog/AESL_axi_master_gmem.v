...
//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.matmul_partition.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.matmul_partition.autotvout_gmem.dat"
...
parameter gmem_transaction_depth = 32'd 768;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/home/hyeon/workspace/array_blcokcyc/component_array_partition_block_cyclic/hls/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/home/hyeon/workspace/array_blcokcyc/component_array_partition_block_cyclic/hls/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
...
integer gmem_port0_fresh_depth[0:1];
integer gmem_port0_blank_depth[0:1];
integer gmem_port1_fresh_depth[0:1];
integer gmem_port1_blank_depth[0:1];
integer gmem_port2_fresh_depth[0:1];
integer gmem_port2_blank_depth[0:1];

initial begin : runtime_depth_init_process
    gmem_port0_fresh_depth[0] = 256;
    gmem_port0_blank_depth[0] = 0;
    gmem_port1_fresh_depth[0] = 256;
    gmem_port1_blank_depth[0] = 0;
    gmem_port2_fresh_depth[0] = 256;
    gmem_port2_blank_depth[0] = 0;
end
//------------------------Read file------------------------ 
...
initial begin : read_file_process 
  ...
  integer i; 
  reg [3 - 1 : 0] port_read_done;
  reg [3 - 1 : 0] port_fill_done;
  transaction_num = 0;
  port_read_done = 'd0;
  port_fill_done = 'd0;
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem_transaction_depth; i = i + 1) begin 
          ...
          ret = $fread(token_tmp,fp);
              ...
              if (port_idx == 2) begin
                  if (port_read_done[port_idx] == 1'b0 && read_idx == gmem_port2_fresh_depth[transaction_num]) begin
                      port_read_done[port_idx] = 1'b1;
                      read_idx = 0;
                  end
                  if (port_read_done[port_idx] == 1'b0 && read_idx < gmem_port2_fresh_depth[transaction_num]) begin
          ret = $fread(token_tmp,fp);
                  ...
                  if (port_fill_done[port_idx] == 1'b0 && port_read_done[port_idx] == 1'b1 && read_idx == gmem_port2_blank_depth[transaction_num]) begin
                      port_fill_done[port_idx] = 1'b1;
                      port_idx = port_idx + 1;
                      read_idx = 0;
                  end
...
integer gmem_cur_trans_depth[0:1];

initial begin : out_depth_init_process
    gmem_cur_trans_depth[0] = 768;
end
//------------------------Write file----------------------- 
  ...
  while(1) begin 
      ...
      for(i = 0; i < gmem_mem_depth; i = i + 1) begin 
          ...
          for (j=0;j<1;j=j+1) begin
              ...
              if (port_idx == 2) begin
                  if (port_write_done[port_idx] == 1'b0 && write_idx == gmem_port2_fresh_depth[transaction_num]) begin
                      port_write_done[port_idx] = 1'b1;
                      write_idx = 0;
                      write_ready = 1'b0;
                  end
                  ...
                  if (port_fill_done[port_idx] == 1'b0 && port_write_done[port_idx] == 1'b1 && write_idx == gmem_port2_blank_depth[transaction_num]) begin
                      write_idx = 0;
                      port_fill_done[port_idx] = 1'b1;
                      port_idx = port_idx + 1;
                  end

...
{
  unsigned int i;
  VITIS_LOOP_359_1: for(i=0;i<25;i++)
    s[i] = 0;
}
# 376 "fips.c"
                                  ...
                                  size_t inlen)
{
  ...
  VITIS_LOOP_384_1: while(pos+inlen >= r) {
    VITIS_LOOP_385_2: for(i=pos;i<r;i++)
  ...
  VITIS_LOOP_392_3: for(i=pos;i<pos+inlen;i++)
    s[i/8] ^= (uint64_t)*in++ << 8*(i%8);
                                   ...
                                   unsigned int r)
{
  ...
  VITIS_LOOP_437_1: while(outlen) {
    if(pos == r) {
      KeccakF1600_StatePermute(s);
      pos = 0;
    }
    VITIS_LOOP_442_2: for(i=pos;i < r && i < pos+outlen; i++)
      *out++ = s[i/8] >> 8*(i%8);
    outlen -= i-pos;
    pos = i;
  }
...
# 754 "fips.c"
__attribute__((sdx_kernel("sha3_256_hw", 0))) void sha3_256_hw(uint8_t h[32], const uint8_t *in, size_t inlen)
{
#line 49 "/home/hyeon/workspace/sha3_syn/run2.tcl"

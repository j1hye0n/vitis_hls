...
//------------------------Parameter----------------------
`define TV_IN_inlen "../tv/cdatafile/c.sha3_256_hw.autotvin_inlen.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter inlen_DEPTH = 1;
reg [31 : 0] inlen_OPERATE_DEPTH = 0;
parameter inlen_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter sha3_256_hw_continue_addr = 0;
parameter sha3_256_hw_auto_start_addr = 0;
parameter inlen_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_inlen [inlen_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_inlen [ (inlen_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * inlen_DEPTH -1 : 0] = '{default : 'hz};
reg inlen_write_data_finish;
...
//write inlen reg
reg [31 : 0] write_inlen_count = 0;
reg [31 : 0] inlen_diff_count = 0;
reg write_inlen_run_flag = 0;
reg write_one_inlen_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & inlen_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_inlen_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (inlen_c_bitwidth, inlen_DEPTH, inlen_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_inlen_run_flag <= 1; 
        end
        else if ((write_one_inlen_data_done == 1 && write_inlen_count == inlen_diff_count - 1) || inlen_diff_count == 0) begin
            write_inlen_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_inlen_data_done === 1) begin
            write_inlen_count = write_inlen_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        inlen_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            inlen_write_data_finish <= 0;
        end
        if (write_inlen_run_flag == 1 && write_inlen_count == inlen_diff_count) begin
            inlen_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_inlen
    ...
    integer j;
    integer k;
    reg [31 : 0] inlen_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = inlen_c_bitwidth;
    ...
    while (1) begin
        ...
        for (k = 0; k < inlen_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (inlen_c_bitwidth < 32) begin
                    inlen_data_tmp_reg = mem_inlen[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < inlen_c_bitwidth) begin
                            inlen_data_tmp_reg[j] = mem_inlen[k][i*32 + j];
                        end
                        else begin
                            inlen_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_inlen[k * four_byte_num  + i]!==inlen_data_tmp_reg) begin
                inlen_diff_count = inlen_diff_count + 1;
                end
            end
        end
...
initial begin : write_inlen
    integer write_inlen_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_inlen_count = 0; check_inlen_count < inlen_OPERATE_DEPTH; check_inlen_count = check_inlen_count + 1) begin
            wait (ongoing_process_number === process_num && process_busy === 0);
            get_ack = 1;
            if (write_inlen_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write inlen data 
                for (i = 0; i < four_byte_num; i = i + 1) begin
                    if (inlen_c_bitwidth < 32) begin
                        inlen_data_tmp_reg = mem_inlen[check_inlen_count];
                    end
                    else begin
                        for (j = 0; j < 32; j = j + 1) begin
                            if (i*32 + j < inlen_c_bitwidth) begin
                                inlen_data_tmp_reg[j] = mem_inlen[check_inlen_count][i*32 + j];
                            end
                            else begin
                                inlen_data_tmp_reg[j] = 0;
                            end
                    ...
                    if(image_mem_inlen[check_inlen_count * four_byte_num  + i]!==inlen_data_tmp_reg) begin
                        image_mem_inlen[check_inlen_count * four_byte_num + i]=inlen_data_tmp_reg;
                        write (inlen_data_in_addr + check_inlen_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, inlen_data_tmp_reg, write_inlen_resp);
                        write_one_inlen_data_done <= 1;
                        @(posedge clk);
                        write_one_inlen_data_done <= 0;
                    end
                end
            end
            process_busy = 0;
...
// Read data from file 
initial begin : read_inlen_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [inlen_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (inlen_c_bitwidth , factor);
  fp = $fopen(`TV_IN_inlen ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_inlen); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < inlen_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_inlen [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin

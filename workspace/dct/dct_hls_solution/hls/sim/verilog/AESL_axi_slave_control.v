...
//------------------------Parameter----------------------
`define TV_IN_input_r "../tv/cdatafile/c.dct.autotvin_input_r.dat"
`define TV_OUT_output_r "../tv/rtldatafile/rtl.dct.autotvout_output_r.dat"
parameter ADDR_WIDTH = 9;
parameter DATA_WIDTH = 32;
parameter input_r_DEPTH = 64;
reg [31 : 0] input_r_OPERATE_DEPTH = 0;
parameter input_r_c_bitwidth = 16;
parameter output_r_DEPTH = 64;
reg [31 : 0] output_r_OPERATE_DEPTH = 0;
parameter output_r_c_bitwidth = 16;
parameter START_ADDR = 0;
parameter dct_continue_addr = 0;
parameter dct_auto_start_addr = 0;
parameter input_r_data_in_addr = 128;
parameter output_r_data_out_addr = 256;
...
assign TRAN_control_write_data_finish = 1 & input_r_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        else if ((write_one_input_r_data_done == 1 && write_input_r_count == input_r_diff_count - 1) || input_r_diff_count == 0) begin
            write_input_r_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_input_r_run_flag == 1 && write_input_r_count == input_r_diff_count) begin
            input_r_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_input_r
    ...
    while (1) begin
        ...
        input_r_diff_count = 0;

        for (k = 0; k < input_r_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (input_r_c_bitwidth < 32) begin
                    input_r_data_tmp_reg = mem_input_r[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < input_r_c_bitwidth) begin
                            input_r_data_tmp_reg[j] = mem_input_r[k][i*32 + j];
                        end
                        else begin
                ...
                if(image_mem_input_r[k * four_byte_num  + i]!==input_r_data_tmp_reg) begin
                input_r_diff_count = input_r_diff_count + 1;
                end
            end
        end
...
initial begin : write_input_r
    ...
    while (1) begin
        ...
        for (check_input_r_count = 0; check_input_r_count < input_r_OPERATE_DEPTH; check_input_r_count = check_input_r_count + 1) begin
            ...
            if (write_input_r_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0; i < four_byte_num; i = i + 1) begin
                    if (input_r_c_bitwidth < 32) begin
                        input_r_data_tmp_reg = mem_input_r[check_input_r_count];
                    end
                    else begin
                        for (j = 0; j < 32; j = j + 1) begin
                            if (i*32 + j < input_r_c_bitwidth) begin
                                input_r_data_tmp_reg[j] = mem_input_r[check_input_r_count][i*32 + j];
                            end
                            else begin
                                input_r_data_tmp_reg[j] = 0;
                            end
                    ...
                    if(image_mem_input_r[check_input_r_count * four_byte_num  + i]!==input_r_data_tmp_reg) begin
                        image_mem_input_r[check_input_r_count * four_byte_num + i]=input_r_data_tmp_reg;
                        write (input_r_data_in_addr + check_input_r_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, input_r_data_tmp_reg, write_input_r_resp);
